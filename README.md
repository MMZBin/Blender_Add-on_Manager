...
addon = AddonManager(__file__, load_folder, locals()) #Create an instance of the AddonManager class
...
def register():
    from .. import addon # Import the AddonManager instance generated in the __init__.py file

    addon.keymap.add(Key(HOGE_OT_Sample, 'F1')) # Set the 'HOGE_OT_Sample' operator to execute when the F1 key is pressed
```
...
- Use the [`KeymapManager`](#keymap_managerpy) class to register shortcut keys.
    - Example: `addon.keymap.add(Key(HOGE_OT_YourOperator, 'A'))`
- You can register, reference, unregister property groups using the [`PropertiesManager`](#properties_managerpy) class.(The initial configuration requires the `addon_name` argument of the `AddonManager` class.)
    - Example
        - Property registration: `addon.property.add(Scene, [("your_properties", YourPropertyGroupClass)])`
        - Property reference:
        ```
            prop = addon.property.get(bpy.context.scene, "your_properties") #Acquiring the property
  ...
  - To avoid circular references, use lazy imports (see the sample code for the [`PropertiesManager`](#properties_managerpy) class for an example).
    - **`__init__(path, target_dirs, local_symbols, addon_name, translation_table, cat_name, is_debug_mode)` method**
        - Arguments:
            ...
            - `local_symbols`: The symbol table of the addon's file (usually the `locals()` function in the `__init__.py` file)
    ...
    **`reload()` Method**
    ...
    - Example
    ```
        addon = AddonManager(__file__, ['operators', 'panels'], locals()) # Instance creation
    ...
    addon = AddonManager(__file__, ['operators', 'panels'], locals(), __name__, translation_dict) # Instance creation
    ...
    ```
    addon = AddonManager(__file__, ['operators', 'panels'], locals(), is_debug_mode=True) # Instance creation
...
- __KeymapManager__ class
    ...
    **`delete(subject) -> bool` method**
    ...
    - Example: `addon.keymap.delete(kms)`
    ...
    **`unregister()` method**
    ...
    - Example: `addon.keymap.unregister()`
...
- __Property__ Class
    ...
    - **`prop_type` property**
        - Returns the class where the property is registered.
    - **`prop` property**
        - Returns the property object.
    - **`prop_name` property**
        - Returns the property name.
    - **`context` attribute**
        - The object where the property is registered
        - If the `Property` object is not associated with a specific object, it becomes `None`.
    - **`get(attr) -> Any` method**
        - Acquires the specified attribute of the property.
            - Argument: `attr`: Attribute name to acquire
            - Return value: Attribute
    - **`set(attr, value)` method**
        - Sets a value to the specified attribute.
            - Arguments
                - `attr`: Attribute name to set the value
                - `value`: Value to set
...
- __PropertiesManager__ Class
    ...
    - **`add(prop_type, properties) -> List[Property]` Method**
        - Adds a property.
        - The prefix specified by the `set_name()` method is attached (`[prefix]_[property name]` form).
            ...
            - Example: `addon.property.add(Scene, [("your_properties", YourPropertyGroupClass)])`
    - **`get_prop(context, attr, is_mangling) -> Any` Method**
        ...
        - By default, if the prefix specified by the `set_name()` method is attached, it retrieves it as is, and if it is not attached, it adds it and retrieves it.
            ...
            - Example: `prop = addon.property.get(bpy.context.scene, "your_properties")`
    - **`delete(prop_name) -> bool` Method**
        ...
        - Example: `addon.property.delete("your_properties")`
    - **`unregister()` Method**
